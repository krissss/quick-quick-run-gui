...
- **自动更新**：通过 GitHub Release 检查、下载并安装新版本。
...
Tag 以 `v` 开头时会触发 GitHub Actions 构建桌面安装包、签名更新包并生成 `latest.json`。首次发布前需要在 GitHub Secrets 中配置 `TAURI_SIGNING_PRIVATE_KEY`，如签名 key 有密码，再配置 `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`。
